               ...
               .map((_, index) => (
                  <div className="flex-grow-0 flex-shrink-0 basis-[calc(16.6667%-16px)] w-[calc(16.6667%-16px)] my-0 mx-2">
                     <SingleInput
                        ...
                        style={inputStyle}
                        className={inputClassName ?? 'w-full h-16 text-center font-dm text-[32px] font-semibold text-[#23262F] bg-neutral8 transition-all duration-300'}
                        disabled={disabled}
                     />
                     <div className={`w-full rounded-lg h-1 ${otpValues[index] ? 'bg-primary1' : 'bg-[#ccd9fe]'}`}></div>
                  </div>
               ))
         }
      </div>
